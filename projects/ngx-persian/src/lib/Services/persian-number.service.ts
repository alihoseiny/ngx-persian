...
export class PersianNumberService implements PersianService {
    ...
    arabicToPersian(value: string): string {
        return value.replace(
            /٤/g,
            PersianNumberService.persianNumbersTable[4]
        ).replace(
            /٥/g,
            PersianNumberService.persianNumbersTable[5]
        ).replace(
            /٦/g,
            PersianNumberService.persianNumbersTable[6]
        );
    }
    ...
    toPersian(value: string | number): string {
        ...
        for (let i = 0; i < PersianNumberService.persianNumbersTable.length; i++) {
            ...
            persianValue = persianValue.replace(regex, PersianNumberService.persianNumbersTable[i]);
        }
        return persianValue;
    }
    ...
    toEnglish(value: string): string {
        ...
        for (let i = 0; i < PersianNumberService.englishNumbersTable.length; i++) {
            ...
            englishValue = englishValue.replace(regex, PersianNumberService.englishNumbersTable[i]);
        }
        return englishValue;
    }
}
