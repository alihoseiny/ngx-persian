 ...
 */
export enum operatorsNames {        // eslint-disable-line no-shadow
    IRANCELL = 'ایرانسل',
    MCI = 'همراه اول',
    TALIA = 'تالیا',
    RIGHTEL = 'رایتل',
...
export class MobilePhoneNumberService {
    ...
    getPhoneNumberOperator(phoneNumber: string, nameInEnglish: boolean = false): string {
        ...
        for (const operator of Object.keys(MobileCodes)) {
            for (const code of MobileCodes[operator as keyof typeof MobileCodes]) {
                if (NormalizedPhoneNumber.indexOf(code) === 0) {
                    return nameInEnglish ? operator : operatorsNames[operator as keyof typeof operatorsNames];
                }
            }
        }
