...
//             for (const code of MobileCodes[operator  as keyof typeof MobileCodes]) {   // Looping through codes for creating all possible phone numbers with
//                 const number = code + baseNumbers.substring(code.length); // A fake phone number with desired code and correct pattern
//                 it(`should return true when input number (${number}) is a valid phone number`, () => {      // eslint-disable-line no-loop-func
...
//                        and nameInEnglish parameter has been set to false`, () => {            // eslint-disable-line no-loop-func
//                     expect(mobilePhoneNumberService.getPhoneNumberOperator(number)).toBe(operatorsNames[operator as keyof typeof operatorsNames]);
//                 });
//             }
//         }
//
...
//                        and nameInEnglish parameter has been set to true`, () => {                 // eslint-disable-line no-loop-func
...
//             for (const code of MobileCodes[operator  as keyof typeof MobileCodes]) {   // Looping through codes for creating all possible phone numbers with
//                 const number = code + baseNumbers.substring(code.length); // A fake phone number with desired code and correct pattern
//                 it(`should return true when input number (${number}) has a valid code and follows correct pattern`, () => {           // eslint-disable-line no-loop-func
